...
export default function ManagedCourses() {
  ...
  const searchCourse = async(hash) => {
    ...
    if(hash && hash.length === 66 && re.test(hash)) {
      ...
      if (course.owner !== "0x0000000000000000000000000000000000000000") {
        ...
        console.log("This is the normalized course", searchedCourse)
        return
      }
  ...
  const fileredCourse = managedCourses.data
  ?.filter((course)=>{
  ...
  return (
    <>
      ...
      <section className="grid grid-cols-1">
        ...
        { fileredCourse }
        { fileredCourse?.length === 0 && <Message>No Courses to Manage</Message>}
      </section>
    </>
  )
}
