...
contract("CourseMarketplace", accounts => {
  ...
  describe("Deactivate course", () => {
    ...
    it("should have status of deactivated and price 0", async () => {
      ...
      const beforeTxOwnerBalance = await getBalance(currectOwner)
      ...
      const afterTxOwnerBalance = await getBalance(currectOwner)
      ...
      assert.equal(toBN(beforeTxBuyerBalance).add(toBN(value)).toString(), afterTxBuyerBalance, "buyerbalance is not correct")
      assert.equal(toBN(beforeTxContractBalance).sub(toBN(value)).toString(), afterTxContractBalance, "contract balance is not correct")

      assert.equal(toBN(beforeTxOwnerBalance).sub(gas).toString(), 
      afterTxOwnerBalance, 
  ...
  describe("Emergency withdraw", ()=>{
    ...
    it("should have contract balance of 0", async()=>{
      const contractBalance = await getBalance(_contract.address)
      assert.equal(toBN(contractBalance), 0, "contract doesn't have 0 balance")
    })
  })
